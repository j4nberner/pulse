...
class OpenAIo3Model(PulseLLMModel):
    """OpenAI o3 model wrapper."""

    def __init__(self, params: Dict[str, Any], **kwargs) -> None:
        """Initializes the OpenAIo3Model with parameters and paths.

        Args:
            params: Configuration dictionary with model parameters.
            **kwargs: Additional optional parameters such as `output_dir` and `wandb`.
        """
        model_name = kwargs.pop("model_name", "OpenAIo3Model")
        ...
        self.client = OpenAI(api_key=os.getenv(params["api_key_name"]))
    ...
    def _generate_standard(
        ...
        num_thinking_tokens = response.usage.output_tokens_details.reasoning_tokens

        thinking_output = ""
        answer_output = response.output_text
