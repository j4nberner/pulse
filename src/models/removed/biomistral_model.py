...
class BioMistralModel(PulseTemplateModel):
    """BioMistral model wrapper using LangChain for prompt templating and inference."""

    def __init__(self, params: Dict[str, Any], **kwargs) -> None:
        """Initializes the BioMistralModel with parameters and paths.
        ...
        self.biomistral_model: Optional[Any] = None
    ...
    def _load_model(self) -> None:
        ...
        try:
            ...
            logger.error("Failed to load BioMistral model: %s", e)
            raise
    ...
    def infer_llm(self, input_text: str) -> Dict[str, Any]:
        ...
        self.biomistral_model.to(self.device)
    ...
    def set_trainer(
        ...
        """
        self.trainer = BioMistralTrainer(
...
class BioMistralTrainer:
    """Trainer class for BioMistralModel."""

    def __init__(
        self, model: BioMistralModel, train_loader, val_loader, test_loader
    ) -> None:
        """
        Initialize the BioMistral trainer. Finetruning is not implemented yet.
        This is a wrapper for inference only.

        Args:
            model (BioMistralModel): The BioMistral model to be trained.
        ...
        self.model = model
        self.biomistral_model = model.biomistral_model
    ...
    def train(self):
        ...
        if self.tuning:
            ...
            optimizer = optim.AdamW(
                self.biomistral_model.parameters(), lr=self.params.get("lr", 1e-4)
            )
            num_epochs = self.params.get("num_epochs", 1)

            self.biomistral_model.train()
            for epoch in range(num_epochs):
                ...
                self.biomistral_model.save_pretrained(self.model_save_dir)
    ...
    def evaluate_batched(self, test_loader: Any, save_report: bool = False) -> float:
        ...
        """
        NotImplementedError(
            "Batch evaluation is not implemented for BioMistralModel. Use evaluate_single instead."
        )
